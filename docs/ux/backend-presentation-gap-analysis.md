...
| Load cards | Load summaries now include current power, configured deadline, fault flag, and next planned action when backend evidence exists, but no full type-specific display model yet. | `display_state`, `status_level`, `status_title`, `status_summary`, `target_label`, `target_current`, `target_required`, `target_percent`, `attention_rank`, richer `source_breakdown`. | Preserve existing fields for compatibility. |
