...
- Replaced the legacy wall of equal-weight site metric cards with:
  ...
  - backend-field-derived attention/opportunity list;
...
| `npm --prefix frontend run test` under Node 22.23.0 | 4 files passed; 21 tests passed |
| `npm --prefix frontend run test:e2e` under Node 22.23.0 | 4 Playwright tests passed |
| `scripts/validate-frontend-bundle` | Passed; bundle size 1,649,743 bytes |
...
| Attention states are backend-authoritative | Partially satisfied: current attention derives from backend fields (`health`, `fault`, `target_status`, `manual_override`), not English parsing. Dedicated backend-ranked `attention[]`/presentation fields remain future work. |
...
- The attention list is a frontend presentation of existing backend fields, not yet the dedicated backend-ranked attention contract from the gap analysis.
...
1. Add optional backend presentation fields for `site_summary.presentation`, `site_summary.attention[]`, and richer `load_list` display fields with schema/contract tests.
