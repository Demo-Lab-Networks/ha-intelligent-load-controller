...
| Python 3.13 + Home Assistant Core 2025.4.0 backend harness | Targeted `scripts/test-backend tests/integration/test_coordinator.py::test_load_list_exposes_backend_card_presentation_fields -q` is blocked in the current shell because no Python 3.13 interpreter is available; syntax/Ruff checks passed locally and GitHub CI provides the Python 3.13 harness after push |
| Frontend | TypeScript check clean; Vitest **36 passed** across 7 files; committed local bundle rebuilt and bundle validator passed at 1,690,855 bytes |
